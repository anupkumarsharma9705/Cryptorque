...
export default function Contact() {
   ...
   * The previous version of this form only ran `setTimeout` and then
   * showed "TRANSMISSION_RECEIVED" — no request was ever made, so every
   * message a visitor "sent" silently vanished. This version actually
   * hands the message off via mailto:, which requires no backend, no
   * API keys, and works the moment the site is deployed.
   *
   * Trade-off: mailto: opens the visitor's own email client rather than
   * sending silently in-page. If you want true in-page delivery (no
   * email client popup), swap this function for EmailJS, Formspree, or
   * a small serverless function — the form UI below doesn't need to
   * change, only what handleSubmit does with `form`.
   */
  const handleSubmit = async (e) => {
    ...
    const mailtoUrl =
      `mailto:${channels[0].value}` +
      `?subject=${encodeURIComponent(subject)}` +
      `&body=${encodeURIComponent(body)}`;

    // Brief delay purely so the "TRANSMITTING..." state is perceptible —
    // not simulating a network call, just avoiding an instant UI flash.
          ...
          {sent ? (
            <div style={{
              ...
              <div style={{
                ...
                Your email app should have opened with the message pre-filled — just hit send there.
              </div>
            </div>
          ) : (
