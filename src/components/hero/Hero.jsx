...
const LINES = [
  ...
  "> stack: java · spring_boot · redis · mysql · docker",
  "> focus: api_security · payment_safety",
...
function Terminal() {
  ...
  return (
    <div style={{ fontFamily: "var(--font-mono)", fontSize: ".72rem", lineHeight: 1.9, color: "var(--text-secondary)", }}>
      {done.map((line, i) => (
        <div key={i} style={{
          color: i === done.length - 1 && finished
            ? "var(--accent-green)"
            : line.includes("domain:") || line.includes("projects:")
              ? "var(--accent-red)"
              : "var(--text-secondary)",
        }}>
          {line}
...
const metrics = [
  { label: "BACKEND_PROJECTS", value: "03", color: "var(--accent-red)" },
  { label: "TECHNOLOGIES", value: "15+", color: "var(--accent-blue)" },
...
export default function Hero() {
  ...
  return (
    <section id="home" className="hero-section" style={{
      minHeight: "var(--vh-full)", display: "flex", flexDirection: "column",
      ...
      <div className="hero-ghost-word" style={{
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{
            ...
            Java backend developer building secure, well-structured server-side systems.
            Focused on{" "}<strong style={{ color: "var(--text-primary)", fontWeight: 500 }}>REST API design</strong>,{" "}
            <strong style={{ color: "var(--text-primary)", fontWeight: 500 }}>Spring Security</strong>, and writing
            backend code that handles edge cases — not just the happy path.
          </p>
      ...
      <style>{`
        .hero-section {
          ...
          overflow: hidden; /* contain the ghost word / speed lines so they can't cause horizontal scroll */
        }
        ...
        /* Same 768px breakpoint used by every other section (About, Skills,
           Education, Certifications, Contact) — this is the one that was
           MISSING here, which is why the hero was the section that broke
           worst on mobile: two columns squeezed into a phone width instead
           of stacking. */
        @media (max-width: 768px) {
          ...
          .hero-ghost-word { font-size: clamp(5rem, 22vw, 9rem) !important; }
        }
        @media (max-width: 480px) {
          .hero-grid h1 { font-size: clamp(1.8rem, 8.5vw, 2.6rem) !important; }
          .hero-ghost-word { display: none; } /* decorative only — pure noise at this size */
        }
      `}</style>
    </section>
  );
